...
def prepare_model(llama2_path, frozen_path, **kwargs):
    ...
    shutil.copy(
        os.path.join(llama2_path, "tokenizer.model"),
        os.path.join(frozen_path, "tokenizer.model"),
    )
    torch.save(
        model.to(args.frozen_dtype).state_dict(), os.path.join(frozen_path, "model.pth")
    )
            ...
            for attn_key in ["v", "q"]:
                ...
                if device_supports_dtype("cpu", original_type):
                    ...
                    lora = (
                        lora_weights[b_key]
                        .to(torch.float32)
                        .mm(lora_weights[a_key].to(torch.float32))
                        * lora_scale
                    )
                    lora = lora.to(original_type)

                subset = get_w_subset(local_path, lora, shards, ci)
                checkpoint[checkpoint_key] = checkpoint[checkpoint_key] + lora[
                    subset
                ].to(torch.bfloat16)
